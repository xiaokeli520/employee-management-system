<template>
  <div class="employee-page">
    <!-- 调试信息（如果不需要可以删除） -->
    <div v-if="false" style="background: #f0f0f0; padding: 8px; margin-bottom: 10px; font-size: 12px">
      当前用户: {{ userStore.user?.name }} | 
      角色: {{ userStore.user?.role }} | 
    ...
    <el-table :data="paginatedList" style="width: 100%">
      ...
      <el-table-column label="权限" width="200">
        <template #default="{ row }">
          ...
          <span v-if="row.permissions.length > 2" style="color: #999">+{{ row.permissions.length - 2 }}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="220">
        <template #default="{ row }">
          <el-button type="primary" size="small" @click="editEmployee(row)">编辑</el-button>
          <!-- 只有管理员才能看到权限按钮 -->
          <el-button 
          ...
          <el-button v-if="canDelete" type="danger" size="small" @click="deleteEmployee(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      <div v-if="permissionUser" class="permission-container">
        <div v-for="(options, group) in permissionGroups" :key="group" class="permission-group">
...
const employeeList = ref([
  { 
    id: 1, name: '张三', position: '前端工程师', department: '技术部', phone: '13800138001',
    username: 'zhangsan', role: 'admin', roleName: '管理员',
  ...
  { 
    id: 2, name: '李四', position: '产品经理', department: '产品部', phone: '13800138002',
    username: 'lisi', role: 'hr', roleName: '人事',
    permissions: ['employee:view', 'employee:add', 'employee:edit', 'employee:delete']
  },
  { 
    id: 3, name: '王五', position: 'UI设计师', department: '设计部', phone: '13800138003',
    username: 'wangwu', role: 'sale', roleName: '销售',
    permissions: ['order:view', 'order:add', 'order:edit', 'order:delete']
  },
  { 
    id: 4, name: '赵六', position: '后端工程师', department: '技术部', phone: '13800138004',
    username: 'zhaoliu', role: 'employee', roleName: '普通员工',
...
const handleSubmit = () => {
  ...
  if (editingId.value) {
    const index = employeeList.value.findIndex(item => item.id === editingId.value)
    if (index !== -1) {
...
const deleteEmployee = (employee: any) => {
  ElMessageBox.confirm(
    ...
    const index = employeeList.value.findIndex(item => item.id === employee.id)
    if (index !== -1) {
...
const savePermissions = () => {
  if (permissionUser.value) {
    const index = employeeList.value.findIndex(u => u.id === permissionUser.value.id)
    if (index !== -1) {
      employeeList.value[index].permissions = [...permissionUser.value.permissions]
    }
